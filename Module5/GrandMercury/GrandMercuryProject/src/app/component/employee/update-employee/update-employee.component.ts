...
export class UpdateEmployeeComponent implements OnInit {
  ...
  buildThisForm() {
    this.formUpdate = this.formBuilder.group({
      ...
      position: [this.employeeUpdate.position == undefined ? "" : this.employeeUpdate.position.id+'', [Validators.required]],
      division: [this.employeeUpdate.division == undefined ? "" : this.employeeUpdate.division.id, [Validators.required]],
      education: [this.employeeUpdate.education == undefined ? "" : this.employeeUpdate.education.id, [Validators.required]],
      dayOfBirth: [this.employeeUpdate.dayOfBirth == undefined ? new Date().toISOString().slice(0, 10) : this.employeeUpdate.dayOfBirth, [Validators.required, this.validatorCustom.check18Age]],
      status: [1]
    })
  }
  ...
  saveDate() {
    ...
    this.employeeService.save(this.formUpdate.value).subscribe(value => {
      this.router.navigateByUrl("/employee/list?message=1").then(resolve => {
        this.employeeUpdate = {};
